...
runs = 5
random_state_v = list(range(1,runs+1))



# Importing the data
df = pd.read_csv('Wb Data/PRM_v1.csv', index_col=0)
...
with open('Outputs/coef_out_ridge_cv.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(X.columns)
    writer.writerow(ridge_df.coef_)

with open('Outputs/coef_out_lasso_cv.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(X.columns)
    writer.writerow(lasso.coef_)

print(random_state_v)
