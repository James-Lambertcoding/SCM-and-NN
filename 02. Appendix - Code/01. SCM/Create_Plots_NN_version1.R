## Creating Plots for NN version 1 Outputs
...
for(k in 1:length(hist_names)){
  ...
  for (i in 1:i_len) {
    ...
    ggplot2::ggsave(filename = paste0("NN_v1/Plots_hist/End Epoch/hist_plot_end_",hist_plot_names[k],"_",i,".png"),plot = plots_lists[[k]][[i]])  
...
for(k in 1:length(prior_names)){
  ...
  ggplot2::ggsave(filename = paste0("NN_v1/Plots_Pred/pred_plot_",model_names[k],".png"),plot = plot_list_pred[[k]])  
...
error_df <- data.frame("model_name" = model_names[1:6],
                       "MSE" = rep(0),
                       "pred" = rep(0))
...
write.csv(x = error_df1, file = "NN_v1/Plots_Pred/error_totals.csv")
...
write.csv(x = pred_all_list[[6]],"Predictions/NN Model 1 Predictions.csv")
