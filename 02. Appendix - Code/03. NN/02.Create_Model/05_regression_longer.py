...
# load the dataset
dataset_name = 'wb_all_extent_reorder.csv'
data_post_name = 'post_int_gdp.csv'
dataset = pd.read_csv(dataset_name, delimiter=',')
post_set = pd.read_csv(data_post_name, delimiter=',')

dataset_rec = dataset.iloc[:,0:42]
post_set_rec = post_set.iloc[:,0:42]

output_actual = dataset.iloc[:,42]

train_dataset = dataset.sample(frac=0.8, random_state=0)
test_dataset = dataset.drop(train_dataset.index)

train_dataset_red = train_dataset.iloc[:,0:42]
test_dataset_red = test_dataset.iloc[:,0:42]
...
def build_model():
    model = keras.Sequential([
        layers.Dense(42, input_shape=(42,), activation='relu'),
...
hist.to_csv('Outputs\History_base_long.csv',index=False)

# test_pred_pre = model.predict(dataset_rec)

# test_predict = pd.DataFrame(test_pred_pre)

# test_predict.to_csv('Outputs\predictions_long.csv',index=False)

# post_pred_pre = model.predict(post_set_rec)
# post_pred = pd.DataFrame(post_pred_pre)

# post_pred.to_csv('Outputs\post_predictions1.csv',index=False)
    
model.save("models/reg_model_long.h5")    
